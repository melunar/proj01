...
define([
    ...
    "layer.ext",
    //"/src/libs/layer/extend/layer.ext.js",
    "qrcode",
    "jquery-ui",
    "global"
], function (jQuery, tools, layer, Model, md5) {
        ...
        var _controller = null,//控制器对象
            ...
            _eventHandler = {
                ...
                "showProduct": function (e) {
                    e.stopPropagation();
                    e.preventDefault();
                    if((localStorage.getItem("companyStatus") == 0) && (localStorage.getItem("userType") === "company")) {
                        layer.msg("贵企业当前处于待审核状态，不能使用该功能", {icon: 2});
                        return;
                    }
                    $(this).addClass("active").siblings().removeClass("active");
                ...
                "showLibs": function (e) {
                   ...
                   /* Model.getBatchListByProductId(_data, function (res) {
                    ...
                    //第一次渲染使用
                    _data0.productId = JSON.parse(localStorage.productList)[0].id;
                    _data0.pageSize = _pageSize;
                    _data0.currentPage = 1;

                    Model.getBatchListByWithPage(_data0, function (res) {
                            if (res.code === 200) {
                                scope.renderPanelLibsList(res.data);
                                $("#pagination").append("&nbsp;&nbsp;&nbsp;&nbsp;一共 " + totalCount + " 条记录");
                                layer.msg(res.message, {icon: 1});
                            } else if (res.code === 500) {
                                layer.msg(res.message, {icon: 2});
                            }
                    ...
                    var totalCount = 0;
                    Model.totalCountBatchBy(_data,function(res) {
                        if (res.code === 200) {
                            totalCount = res.data;
                            layer.msg(res.message, {icon: 1});
                            tools.setPagination(totalCount, "#pagination", _pageSize, _pageIndex, function (nextPage) {
                                ...
                                //console.log("pagenation",_data);
                                Model.getBatchListByWithPage(_data, function (res) {
                                        if (res.code === 200) {
                                            scope.renderPanelLibsList(res.data);
                                            $("#pagination").append("&nbsp;&nbsp;&nbsp;&nbsp;一共 " + totalCount + " 条记录");
                                            //layer.msg(res.message, {icon: 1});
                                        ...
                                        layer.msg("服务器出错，加载patch列表失败", {icon: 2});
                                    }
                                );
                            });
                        } else if (res.code === 500) {
                ...
                "showWord": function (e) {
                    ...
                    $(this).addClass("active").siblings().removeClass("active");
                    //alert(JSON.parse(localStorage.productList)[0].id);
                    scope.renderPanelWord(function() {
                        $("#radio0031").trigger("click");//触发全部搜索
                    });

                },

                "showReminderUploadContent":function (e) {
                    e.stopPropagation();
                    e.preventDefault();
                    if($(".reminder-upload-content")) {
                        $(".reminder-upload-content").eq(0).toggleClass("hide");
                    }
                },
                //获取留言
                "searchWord": function (e) {
                   ...
                   // e.preventDefault();
                    ...
                    //_data.type = $("#wordTypeSelectList option:selected").val();
                    _data.star = $("#wordStarSelectList option:selected").val();
                    _data.isRead = $("input[name='wordIsRead']:checked").data("value");

                    var _data0 = {};


                    _data.date1 = $("input[name='date1']").val();
                    _data0.date1 = $("input[name='date1']").val();
                    _data.date2 = $("input[name='date2']").val();
                    _data0.date2 = $("input[name='date2']").val();

                    var dateAStr=_data.date1;
                    var arrA=dateAStr.split("-");
                    var dateA=new Date(arrA[0],arrA[1],arrA[2]);
                    var dateAT=dateA.getTime();
                    var dateBStr=_data.date2;
                    var arrB=dateBStr.split("-");
                    var dateB=new Date(arrB[0],arrB[1],arrB[2]);
                    var dateBT=dateB.getTime();
                    if(dateAT > dateBT) {
                        layer.msg("结束日期不能小于起始日期", {icon: 2});
                        return;
                    }

                    //第一次渲染使用
                    _data0.companyId = localStorage.getItem("companyId");
                    //_data0.type = $("#wordTypeSelectList option:selected").val();
                    _data0.star = $("#wordStarSelectList option:selected").val();
                    _data0.isRead = $("input[name='wordIsRead']:checked").data("value");
                    _data0.pageSize = _pageSize;
                    _data0.currentPage = 1;

                    var totalCount = 0;
                    Model.getWordListByWithPage(_data0, function (res) {
                            if (res.code === 200) {
                                scope.renderPanelWordList(res.data);
                                $("#pagination").append("&nbsp;&nbsp;&nbsp;&nbsp;一共 " + totalCount + " 条记录");
                                //layer.msg(res.message, {icon: 1});
                            } else if (res.code === 500) {
                                layer.msg(res.message, {icon: 2});
                    ...
                    Model.totalCountWord(_data,function(res) {
                        if (res.code === 200) {
                            totalCount = res.data;
                            layer.msg(res.message, {icon: 1});
                            //alert(res.message);
                            tools.setPagination(totalCount, "#pagination", _pageSize, _pageIndex, function (nextPage) {
                                ...
                                _data.pageSize = _pageSize;
                                console.log("pagenation",_data);
                                Model.getWordListByWithPage(_data, function (res) {
                                        if (res.code === 200) {
                                            scope.renderPanelWordList(res.data);
                                            $("#pagination").append("&nbsp;&nbsp;&nbsp;&nbsp;一共 " + totalCount + " 条记录");
                                            //layer.msg(res.message, {icon: 1});
                                        ...
                                        layer.msg("服务器出错，加载留言列表失败", {icon: 2});
                                    }
                                );
                            });
                        } else if (res.code === 500) {
                ...
                //留言详情的显示和隐藏
                "showOneWord": function (e) {
                    e.stopPropagation();
                    e.preventDefault();
                    var $r = $(this).next(".wordListItemDetail").eq(0);
                    //console.log($r);
                    if($r.hasClass("hide")) {
                        $r.fadeIn(700).removeClass("hide");
                    } else {
                        $r.addClass("hide");
                    }
                    if($(this).data("isread") == 0) {
                        ...
                        Model.updateWord(_data,function(res) {
                            //不做信息提示 这里把留言标为已读告诉后台
                        })
                    }
                },
                //删除留言
                "delWord" :function (e) {
                    ...
                    _data = {};
                    _data.id = $(this).data("id");
                    layer.msg("确定删除该留言？将无法恢复！！！", {
                        time: 0,//不自动关闭
                        btn: ["确定", "取消"],
                        yes: function (index) {
                            layer.close(index);
                            console.log(_data);
                            Model.deleteWord(_data, function (res) {
                                if (res.code === 200) {
                                    layer.msg(res.message, {icon: 1});
                                    setTimeout(function () {
                                        $("#radio0031").click();
                ...
                //删除产品日志
                "delOneLog" :function (e) {
                    e.stopPropagation();
                    e.preventDefault();

                    _data = {};
                    _data.id = $(this).data("id");
                    layer.msg("确定删除该项？将无法恢复！！！", {
                        time: 0,//不自动关闭
                        btn: ["确定", "取消"],
                        yes: function (index) {
                            layer.close(index);
                            console.log(_data);
                            Model.deleteProductLog(_data, function (res) {
                                if (res.code === 200) {
                                    layer.msg(res.message, {icon: 1});
                                    setTimeout(function () {
                                        $("button[data-handler='showLogs']").click();
                                    }, loginDelayTime);
                                } else if (res.code === 500) {
                                    layer.msg(res.message, {icon: 2});
                                }
                ...
                "seeDetailProduct": function (e) {
                    ...
                    $(this).closest("tr").next("tr").fadeIn(700).removeClass("hide");
                },
                //删除产品
                ...
                "addStep": function (e) {
                    ...
                    if (_stepsCount <= 10) {
                        ...
                        $("#delStep").fadeIn(700).removeClass("hide");
                        if (_stepsCount >= 11) {
                ...
                "delStep": function (e) {
                    ...
                    if (_stepsCount >= 4) {
                        ...
                        _stepsCount--;
                        $("#addStep").fadeIn(700).removeClass("hide");
                        if (_stepsCount <= 3) {
                ...
                "turnToAddProduct": function (e) {
                    e.stopPropagation();
                    e.preventDefault();
                    $(".add-product-box").eq(0).fadeIn(700).removeClass("hide");
                    //按钮切换显示
                    _btn3Toggle();
                },
                //取消添加一个产品
                ...
                "turnToAddBatch": function (e) {
                    e.stopPropagation();
                    e.preventDefault();
                    $(".batchPanel").eq(0).fadeIn(700).fadeIn(700).removeClass("hide");
                    //按钮切换显示
                    _btn3Toggle();
                },
                //取消添加一个产品批次
                ...
                "submitAddBatch": function (e) {
                    ...
                    _data.info = $("input[data-name='info']").val();
                    console.log(_data);
                    if (!_data.count) {
                ...
                "delOneProductBatch": function (e) {
                    ...
                    var code = $(this).closest("tr").find("td[data-name=code]").data("value");
                    var id = $(this).data("id");

                    layer.msg("确定批次'" + code + "',将无法恢复！！！", {
                        time: 0,//不自动关闭
                        btn: ["确定", "取消"],
                        yes: function (index) {
                            layer.close(index);
                            _data = {};
                            _data.id = id;
                            Model.deleteBatch(_data, function (res) {
                                if (res.code === 200) {
                                    layer.msg(res.message, {icon: 1});
                                    setTimeout(function () {
                                        $(".tab-item[data-type='libs']").click();
                                    }, loginDelayTime);
                                } else if (res.code === 500) {
                                    layer.msg(res.message, {icon: 2});
                                }
                ...
                "ShowOneProductBatch": function (e) {
                    ...
                    $(this).closest("tr").next("tr").fadeIn(700).removeClass("hide").find(".QRcode-big-box").text("");
                    var batchId = $(this).closest("tr").data("id");

                  //  var text = $(this).closest("tr").find("[data-name='code']").data("value");
                    var text = "http://" + localIp + ":8089/src/module/trace/log.html?companyId=" + localStorage.getItem("companyId") + "&batchId=" + batchId;
                    //绘制二维码
                ...
                "downloadQRCode": function (e) {
                    ...
                    var code = $(this).closest("tr").prev().find("[data-name='code']").text().trim(),//批次号
                        filename = '二维码，产品批次编号：' + code + " " + (new Date()).getTime() + '.' + type;
// download
                ...
                "searchProduct": function (e) {
                    ...
                    var _data0 = {};
                    _data.productId = $("#localProductList").val();
                    _data0.productId = $("#localProductList").val();
                    ...
                    if ($("input[name='checkProductStatus']:checked").data("value")) {
                        _data.status = $("input[name='checkProductStatus']:checked").data("value");
                        _data0.status = $("input[name='checkProductStatus']:checked").data("value");
                    }
                ...
                "changePassword": function (e) {
                    ...
                    layer.open({
                        ...
                        fix: true, //固定
                        maxmin: true,
                        content: "inc/changePassword.html",
                        success: function (layero, index) {
                            var $panel = $(window.frames[name ^= "layui-layer-iframe"].document).find("#changePassword");

                            $($panel).find("#changePasswordSubmit").on("click", function () {
                                _data = {};
                                var p0 = $($panel).find("[name=password0]").val();
                ...
                "fileUploadLogPic": function (e) {
                    ...
                    layer.open({
                        ...
                        success: function (layero, index) {
                            ...
                            $("button[data-handler='fileUploadLogPic']").text("重新上传").next("span").text("已上传");
                        }
                    });
                ...
                "submitAddLog": function (e) {
                    ...
                    if($(r).find("[data-name=time]").val().trim() === "") {
                        layer.tips("选择时间","#showLog input[data-name='time']");
                        return;
                    }
                    if($("button[data-handler='fileUploadLogPic']").next("span span").is(".red")) {
                        layer.msg("没有上传图片",{icon: 2});
                        return;
                    }
                    if($(".product-item-box span").find(".red").length > 0) {
                        layer.msg("没有上传图片",{icon: 2});
                        return;
                    }
                    var $flag = $('input:radio[name="selectBatch"]:checked');
                    ...
                    _data.productId = $("#localProductList option:selected").val();
                    ...
                    Model.addProductLog(_data,function(res) {
                        if (res.code === 200) {
                            ...
                            setTimeout(function () {
                                ...
                                $("button[data-handler='cancelAddLog']").click();
                            }, loginDelayTime)
                        } else if (res.code == 500) {
                            layer.msg(res.message, {icon: 2});
                        }
            ...
            _btn3Toggle = function () {
                ...
                btns.eq(1).fadeIn(700).removeClass("hide");
                btns.eq(2).fadeIn(700).removeClass("hide");
            ...
            _btn3ToggleBack = function () {
                ...
                btns.eq(0).fadeIn(700).removeClass("hide");
            },
            /**
             * 绑定事件
             * @param selector
        ...
        scope.renderPanelDetail = function (data) {
            ...
            tools.loadTemplate(_tabPanelsTpl, function (compiler) {
                ...
                //生成公司二维码
                //var tt = "http://172.17.66.190:8089/src/module/trace/index.html?companyId=" + localStorage.getItem("companyId");
                var tt = "http://" + localIp + ":8089/src/module/trace/index.html?companyId=" + localStorage.getItem("companyId");
                $("#QR-code").qrcode({
                    text: tools.utf16to8(tt),
                    width: 200,     //设置宽度
                    height: 200,     //设置高度
                    typeNumber: -1,      //计算模式
                    background: "#fff",//背景颜色
        ...
        scope.renderPanelLogsList = function (data) {
            ...
            $(".timeTamp").each(function () {
                $(this).html(tools.mills2str(parseInt($(this).text())));
            });
            layer.photos({
        ...
        scope.renderPanelLogsAdd = function () {
            ...
            tools.loadTemplate(_tabPanelsTpl, function (compiler) {
                ...
                //获取产品类型
                var pt = $("#localProductList option:selected").data("type");
               // alert(pt);
        ...
        //留言板面板
        scope.renderPanelWord = function (callback) {
            tools.loadTemplate(_tabPanelsTpl, function (compiler) {
                var htmlStr = compiler({
                    "word": true
                });
                $("#panel-content").eq(0).html(htmlStr);
                setTimeout(function() {
                    callback();
                },200);
        ...
        //留言板列表
        scope.renderPanelWordList = function (data) {
            tools.loadTemplate(_tabPanelsTpl, function (compiler) {
                var htmlStr = compiler({
                    "wordList": true,
                    "data": data
                });
                $(".wordListPanel").eq(0).html(htmlStr);
                $(".wordListItem").each(function () {
                    if($(this).data("isread") == "0") {
                        $(this).addClass("not-read");
                    }
                });
                _bindEvents(".wordListPanel");
            });
                 $(".wordListPanel .timestamp").each(function () {
             $(this).text(tools.mills2datetime(parseInt($(this).text())));
             });
